...
class AllPrivateGradleProjectImportHandler : AbstractGradleImportHandler<AllPrivateModel>() {
    override val compilerPluginId = AllPrivateCommandLineProcessor.PLUGIN_ID
    override val pluginName = "allprivate"
    override val annotationOptionName = AllPrivateCommandLineProcessor.ANNOTATION_OPTION.optionName
    override val pluginJarFileFromIdea = PathUtil.kotlinPathsForIdeaPlugin.allPrivatePluginJarPath
    override val modelKey = AllPrivateProjectResolverExtension.KEY

    override fun getAnnotationsForPreset(presetName: String): List<String> {
        for ((name, annotations) in AllPrivateCommandLineProcessor.SUPPORTED_PRESETS.entries) {
            if (presetName == name) {
                return annotations
            }
        }
